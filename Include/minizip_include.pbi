...
#IsMINIZIP	=	0	; 0 = Custom-minizip -> no compression, reduce ~35KB / Peace/TST - 05.07.2020
...
CompilerSelect #PB_Compiler_Processor
	CompilerCase #PB_Processor_x86
		#miniziplib$ = "minizip_x86.lib"
		!extrn __chkstk
		!public __chkstk as "___chkstk"
	CompilerCase #PB_Processor_x64
		#miniziplib$ = "minizip_x64.lib"
CompilerEndSelect
ImportC #miniziplib$
...
CompilerIf	#IsMINIZIP
	CompilerIf #PB_Compiler_Version >= 540
		UseCRC32Fingerprint()
		Procedure.l CRC32Fingerprint(*source, sourceLen)
			ProcedureReturn Val("$" + Fingerprint(*source, sourceLen, #PB_Cipher_CRC32))
		EndProcedure
		Procedure.l CRC32FileFingerprint(FileName.s)
			ProcedureReturn Val("$" + FileFingerprint(FileName, #PB_Cipher_CRC32))
		EndProcedure
	CompilerEndIf
CompilerEndIf

CompilerIf Defined(MakeSureDirectoryPathExists, #PB_Procedure) = #False
	Procedure MakeSureDirectoryPathExists(Dir.s)
		If Dir = ""
		...
		Else
			...
			If (Len(Dir) < 3) Or FileSize(Dir) = -2 Or GetPathPart(Dir)= Dir
				ProcedureReturn #False
			EndIf
			MakeSureDirectoryPathExists(GetPathPart(Dir))
			CreateDirectory(Dir)
...
Procedure ZIP_GetFilesCount(Zip_FileName.s)
	...
	ProcedureReturn #False
EndProcedure
Procedure ZIP_IsZipArchive(ZipFilename.s)
	Protected file_api.zlib_filefunc_def
...
EndProcedure
Procedure ZIP_ExtractFile(Zip_FileName.s, FileNumber, Output_Path.s, CreatePath = #True, Callback = 0)
	...
	If FileNumber > 0 And FileNumber <= ZIP_GetFilesCount(Zip_FileName)
		If Handle
...
CompilerIf	#IsMINIZIP
	Procedure ZIP_GetFileNumber(Zip_FileName.s, FileName.s)
		Protected Dim filename_buffer.c(#MAX_PATH + 1)
		...
		If Handle
			...
			If Not unzGoToFirstFile(Handle)
				...
				If FileInfo\FileName = FileName
					...
					ProcedureReturn 1
				EndIf
				fCount = ZIP_GetFilesCount(Zip_FileName)
				For i = 2 To fCount
					unzGoToNextFile(Handle)
					unzGetCurrentFileInfo(Handle, FileInfo, @filename_buffer(), #MAX_PATH, 0, 0, 0, 0)
					FileInfo\FileName = UCase(PeekS(@filename_buffer(), -1, #PB_UTF8))
		...
		EndIf
	EndProcedure
	Procedure ZIP_BrowseDirectory(sPath.s, List ZIP_DirList.s(), List ZIP_FileList.s())
			...
			While NextDirectoryEntry(dirID)
				...
				Else
					...
					If (sDirName <> ".") And (sDirName <> "..")
						...
						ZIP_BrowseDirectory(sPath + sDirName, ZIP_DirList(), ZIP_FileList())
					EndIf
				EndIf
			Wend
		...
		EndIf
	EndProcedure
	Procedure ZIP_PackMemory(*source, sourceLen = #PB_Default, level = #Z_DEFAULT_COMPRESSION)
		...
		EndIf
	EndProcedure
	Procedure ZIP_UnpackMemory(*source, *dest)
		...
		EndIf
	EndProcedure
	Procedure.s ZIP_GetFileInfo(Zip_FileName.s, FileNumber, *FileInfo.unz_file_info = 0)
		Protected Dim filename_buffer.c(#MAX_PATH + 1)
		...
		ProcedureReturn ""
	EndProcedure
	Procedure ZIP_SetPassword(password.s = "")
		Shared ZIP_Password
		ZIP_Password = password
	EndProcedure
	Procedure ZIP_IsPasswordRequired(Zip_FileName.s, FileNumber)
		Protected zfi.unz_file_info
		
		ZIP_GetFileInfo(Zip_FileName, FileNumber, @zfi)
		If zfi\flag & %1
			ProcedureReturn #True
		EndIf
		ProcedureReturn #False
	EndProcedure
	Procedure.s ZIP_GetFileComment(Zip_FileName.s)
		Protected GlobalInfo.unz_global_info
		Protected file_api.zlib_filefunc_def
		fill_fopen_filefunc(@file_api)
		file_api\zopen_file = @fopen_file_func()
	...
	EndProcedure
	Procedure ZIP_ExtractArchiv(Zip_FileName.s, Output_Path.s, CreatePath = #True, Callback = 0)
		Protected Dim filename_buffer.c(#MAX_PATH + 1)
		Protected FileInfo.unz_file_info
		...
		If Handle
			divider2 = 100 / ZIP_GetFilesCount(Zip_FileName)
			If Right(Output_Path, 1) <> "\" : Output_Path + "\" : EndIf
			While firstFile <> -1
				count + 1
				If Callback
	...
	EndProcedure
	Procedure ZIP_CatchFile(Zip_FileName.s, FileNumber, Callback = 0)
		Protected Dim filename_buffer.c(#MAX_PATH + 1)
		Protected FileInfo.unz_file_info
	...
	EndProcedure
	Procedure ZIP_FileAdd(ZipHandle, Filename.s, Archive_Filename.s, Compression = #Z_DEFAULT_COMPRESSION, Callback = 0)
		...
		ProcedureReturn #False
	EndProcedure
	Procedure ZIP_DirAdd(ZipHandle, Directory.s, Compression = #Z_DEFAULT_COMPRESSION, Callback = 0)
		...
		If FileSize(Directory) = -2
			ZIP_BrowseDirectory(Directory, ZIP_DirList(), ZIP_FileList())
			ForEach ZIP_DirList()
			...
			ForEach ZIP_FileList()
				...
				If ZIP_Password <> ""
					result = zipOpenNewFileInZip3(ZipHandle, Archive_Filename, @zfi, #Null, 0, #Null, 0, #Null, #Z_DEFLATED, Compression, 0, 15, 8, 0, ZIP_Password, CRC32FileFingerprint(ZIP_FileList()))
				Else
					result = zipOpenNewFileInZip2(ZipHandle, Archive_Filename, @zfi, #Null, 0, #Null, 0, #Null, #Z_DEFLATED, Compression, 0)
				EndIf
				If Not result
		...
		EndIf
	EndProcedure
	Procedure ZIP_MemAdd(ZipHandle, *source, sourceLen, Archive_Filename.s, Compression = #Z_DEFAULT_COMPRESSION, Callback = 0)
		Protected zfi.zip_fileinfo
		...
		If ZIP_Password <> ""
			result = zipOpenNewFileInZip3(ZipHandle, Archive_Filename, @zfi, #Null, 0, #Null, 0, #Null, #Z_DEFLATED, Compression, 0, 15, 8, 0, ZIP_Password, CRC32Fingerprint(*source, sourceLen))
		Else
			result = zipOpenNewFileInZip2(ZipHandle, Archive_Filename, @zfi, #Null, 0, #Null, 0, #Null, #Z_DEFLATED, Compression, 0)
		EndIf
		If Not result
